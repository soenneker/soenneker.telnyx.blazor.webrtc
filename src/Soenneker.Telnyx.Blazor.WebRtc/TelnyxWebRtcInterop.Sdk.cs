...
public sealed partial class TelnyxWebRtcInterop
{
    ...
    public ValueTask SetIceServers(string id, List<TelnyxIceServer> servers, CancellationToken cancellationToken = default) => Execute(t => InvokeVoidAsync("setIceServers", t, id, JsonUtil.Serialize(servers, GetJsonTypeInfo<List<TelnyxIceServer>>())), cancellationToken);
    ...
    public ValueTask Login(string id, TelnyxLoginOptions? options, CancellationToken cancellationToken = default) => Execute(t => InvokeVoidAsync("login", t, id, options is null ? null : JsonUtil.Serialize(options, GetJsonTypeInfo<TelnyxLoginOptions>())), cancellationToken);
    ...
    public ValueTask<string?> RunPreCallDiagnosis(TelnyxPreCallDiagnosisOptions options, CancellationToken cancellationToken = default) => Execute(t => InvokeAsync<string?>("runPreCallDiagnosis", t, JsonUtil.Serialize(options, GetJsonTypeInfo<TelnyxPreCallDiagnosisOptions>())), cancellationToken);
    ...
    public ValueTask SendAiConversationMessage(string id, TelnyxFunctionCallOutput item, CancellationToken cancellationToken = default) => Execute(t => InvokeVoidAsync("sendAiConversationMessage", t, id, JsonUtil.Serialize(item, GetJsonTypeInfo<TelnyxFunctionCallOutput>())), cancellationToken);
    ...
    public ValueTask StartScreenShare(string id, TelnyxCallOptions options, IJSObjectReference? localStream, IJSObjectReference? remoteStream,
        ...
        Execute(t => InvokeVoidAsync("startScreenShare", t, id, JsonUtil.Serialize(options, GetJsonTypeInfo<TelnyxCallOptions>()), localStream, remoteStream, localElement, remoteElement), cancellationToken);
}
